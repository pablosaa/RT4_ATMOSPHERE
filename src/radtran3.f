...
c      PARAMETER (MAXV=64, MAXM=4096, MAXLM=201*256)
C     PARAMETER (MAXV=64, MAXM=4096, MAXLM=201*512)
      PARAMETER (MAXV=90, MAXM=8100, MAXLM=201*8100)
      PARAMETER (MAXLEG=256, MAXLAY=200)
      PARAMETER (MAXSBUF=MAXLAY*2*MAXM , MAXDBUF=MAXLAY*2*MAXV)
      ...
      IF (SRC_CODE .EQ. 1 .OR. SRC_CODE .EQ. 3) THEN
        IF (GROUND_TYPE .NE. 'L') THEN
          WRITE (27,*) 'Solar case requires Lambertian surface'
          STOP
        ENDIF
      ENDIF
     ...
     .    MAXDBUF .LT. (AZIORDER+1)*2*N*NUM_LAYERS) THEN
          WRITE (27,'(1X,A,I3)') 'Direct source buffer size exceeded.'
          STOP
      ENDIF
     ...
     .     'Too many layers.  Maximum number :', MAXLAY
      ...
      IF ((NUM_LAYERS+1)*N*N .GT. MAXLM) THEN
          WRITE (27,'(1X,A,A,I3)') 'Matrix layer size exceeded.',
...
C                   Read scattering file in
     ...
     .                             LEGENDRE_COEF, EXTINCT, SCATTER)
              IF (NUMLEGEN .GT. MAXLEG) THEN
                  WRITE (27,*)  'Too many Legendre terms.'
                  STOP
              ENDIF
