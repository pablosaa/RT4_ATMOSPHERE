     ...
     .                 NOUTLEVELS, OUTLEVELS,NUMAZIMUTHS,ITIME)
...
C
      ...
      INTEGER   NSTOKES, NUMMU, AZIORDER,kk
      INTEGER   NUM_LAYERS, SRC_CODE
      INTEGER   NOUTLEVELS, OUTLEVELS(MAXLAY), NUMAZIMUTHS, ITIME
...
C       write(*,*)'reading layers'  ! PSG: *=27
      CALL READ_LAYERS (LAYER_FILE, MAXLAY, NUM_LAYERS,
     .                  HEIGHT, TEMPERATURES,
     .                  GAS_EXTINCT, SCAT_FILES)
C       write(*,*)'read layers,now calling RT',shape(UP_RAD), UP_RAD(1:5) ! PSG: *=27
...
C      !write(33,*) 'kk=[',UP_RAD,'];'  ! PSG: still to be tested
      !close(33)   ! PSG: close matlab ascii file
c     N = NUMMU*(AZIORDER+1)*NOUTLEVELS
c      CALL CONVERT_OUTPUT (UNITS, OUTPOL, NSTOKES, N, 
c     .                     WAVELENGTH, 0, UP_RAD)
     ...
     .                    MU_VALUES, UP_FLUX, DOWN_FLUX,
     .                    UP_RAD, DOWN_RAD, ITIME)  ! PSG: ITIME added
      !write(*,*) 'After exit OUTPUT_FILE ', shape(UP_RAD),UP_RAD(1:20)  ! PSG: for debugging
      
c        do kk=1,nummu
c       TB_RT3(kk)=UP_RAD(1,kk,1,1)
c         enddo
c         write(24,*)'mu_values' ,mu_values,TB_RT3
c      call interp_RT3(MU_values,umu,UP_RAD(1,TB_RT3,0.3,TB_output,maxv)

      END



       subroutine RT3_oneangle(angle_view,
     .          NSTOKES, NUMMU,AZIORDER, MU_VALUES,
     .                 SRC_CODE, LAYER_FILE, OUT_FILE,
     .                 QUAD_TYPE, DELTAM, DIRECT_FLUX, DIRECT_MU,
     .                 GROUND_TEMP, GROUND_TYPE,
     .                 GROUND_ALBEDO, GROUND_INDEX,
     .                 SKY_TEMP, WAVELENGTH, UNITS, OUTPOL,
     .           NOUTLEVELS, OUTLEVELS,NUMAZIMUTHS,I1,I2,Upar,Vpar)
      ...
      REAL*8    GROUND_TEMP, GROUND_ALBEDO, angle_view
      COMPLEX*16  GROUND_INDEX
      REAL*8    SKY_TEMP, WAVELENGTH, MAX_DELTA_TAU
      REAL*8    DIRECT_FLUX, DIRECT_MU
      REAL*8    MU_VALUES(MAXV)
     ...
     $TB_RT3(MAXV),TB_output
      ...
      CHARACTER*164 OUT_FILE
      CHARACTER*64 SCAT_FILES(MAXLAY)
       real*8 Field_1,field_2,I1,I2,Upar,Vpar
     ...
     .                    UP_RAD, DOWN_RAD, ITIME)   ! PSG: ITIME added
      ...
      shift=NSTOKES-1
      Field_1=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+Nstokes*i_loc-shift)
      Field_2=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+Nstokes*
     $ (i_loc+1)-shift)
c       write(23,*)'field1',Field_1,field_2
      shift=NSTOKES-2 
      I1=Field_1+(Field_2-Field_1)*(dcos(angle_view)-mu_values(i_loc))/
     $ (mu_values(i_loc+1)-mu_values(i_loc))
      Field_1=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+Nstokes*i_loc-shift)
      Field_2=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+
     $ Nstokes*(i_loc+1)-shift)
      I2=Field_1+(Field_2-Field_1)*(dcos(angle_view)-mu_values(i_loc))/
     $ (mu_values(i_loc+1)-mu_values(i_loc))
      ...
      Field_2=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+
     $ Nstokes*(i_loc+1)-shift)
      Upar=Field_1+(Field_2-Field_1)*
     $(dcos(angle_view)-mu_values(i_loc))/
     $ (mu_values(i_loc+1)-mu_values(i_loc))
      shift=NSTOKES-4
      Field_1=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+Nstokes*i_loc-shift)
      Field_2=DOWN_RAD(NSTOKES*nummu*(Noutlevels-1)+
     $ Nstokes*(i_loc+1)-shift)
      Vpar=Field_1+(Field_2-Field_1)*
     $(dcos(angle_view)-mu_values(i_loc))/
     $ (mu_values(i_loc+1)-mu_values(i_loc))
       endif
...
50        CONTINUE
     ...
     .                    UP_RAD, DOWN_RAD, ITIME)   ! PSG: ITIME added
      ...
      INTEGER  NOUTLEVELS, OUTLEVELS(*), NUMAZIMUTHS, ITIME   ! PSG: ITIME added
      REAL*8   GROUND_TEMP, GROUND_ALBEDO
      REAL*8   SKY_TEMP, WAVELENGTH
      REAL*8   DIRECT_FLUX, DIRECT_MU
      ...
      REAL*8 OUTVAR(NUMMU,NSTOKES,2,NOUTLEVELS)   ! PSG: variable to pass NetCDF
...
C     Output the parameters
     ...
     .        '   OUTPUT_POLARIZATION=', OUTPOL  
         ...
         IF (UNITS(1:1) .EQ. 'T') THEN
            FORM1 = '(F8.3,1X,F5.1,1X,F8.5,4(1X,F8.2),:)' ! PSG: 4(1X,F7.2),:)
         ELSE
...
C     PSG: end of storing NetCDF file.
